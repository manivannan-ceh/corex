...
function StatCard({ label, value, icon: Icon, gradient, glow, borderColor }: StatCardProps) {
  return (
    <div className="card-accent p-6 flex items-center gap-5 transition-all duration-200 hover:scale-[1.01] cursor-default"
         style={{ borderColor }}>
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-secondary text-sm mt-1">Here's what's happening with your releases.</p>
        </div>
      ...
      {pLoad ? <Spinner fullPage /> : (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard label="Total Projects"  value={projects?.length ?? 0}  icon={FolderKanban}
            gradient="linear-gradient(135deg,#6366f1 0%,#4f46e5 100%)" glow="rgba(99,102,241,0.4)"  borderColor="rgba(99,102,241,0.2)" />
          <StatCard label="Total Releases"  value={totalReleases}           icon={Package}
            gradient="linear-gradient(135deg,#10b981 0%,#059669 100%)"  glow="rgba(16,185,129,0.4)" borderColor="rgba(16,185,129,0.2)" />
          <StatCard label="Recent Uploads"  value={recentReleases?.length ?? 0} icon={TrendingUp}
            gradient="linear-gradient(135deg,#8b5cf6 0%,#7c3aed 100%)"  glow="rgba(139,92,246,0.4)" borderColor="rgba(139,92,246,0.2)" />
        </div>
      )}
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="font-semibold text-primary text-sm">Recent Uploads</h2>
          </div>
        ...
        {rLoad ? (
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-xs text-muted mt-1 mb-4">Upload your first APK to get started</p>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Project', 'Version', 'Channel', 'Uploaded'].map((h) => (
                    <th key={h} className="text-left px-6 py-3 text-xs font-semibold text-muted tracking-wide uppercase">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {recentReleases.map((r) => (
                  <tr key={r.id} className="table-row-hover cursor-pointer"
                    ...
                    <td className="px-6 py-3.5 text-secondary font-medium">
                      {projectMap.get(r.project_id) ?? `Project #${r.project_id}`}
                    </td>
                    <td className="px-6 py-3.5">
                      <span className="text-primary font-mono text-sm">{r.version_name}</span>
                      <span className="text-muted text-xs ml-1.5 font-mono">({r.version_code})</span>
                    </td>
                    <td className="px-6 py-3.5"><ChannelBadge channel={r.channel} /></td>
                    <td className="px-6 py-3.5 text-muted text-xs">{formatDate(r.uploaded_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
